...
{
  public class CustomHolisticTrackingSolution : ImageSourceSolution<HolisticTrackingGraph>
  {
    ...
    protected override void OnStartRun()
    {
      ...
      _rigAnimator = GetComponent<RigAnimator>();
    }
    ...
    protected override IEnumerator WaitForNextValue()
    {
      ...
      {
        var _ = graphRunner.TryGetNext(out poseDetection, out poseLandmarks, out faceLandmarks, out leftHandLandmarks, out rightHandLandmarks, out poseWorldLandmarks, out leftHandWorldLandmarks, out rightHandWorldLandmarks, out segmentationMask, out poseRoi, true);
      }
      else if (runningMode == RunningMode.NonBlockingSync)
      {
        yield return new WaitUntil(() =>
          graphRunner.TryGetNext(out poseDetection, out poseLandmarks, out faceLandmarks, out leftHandLandmarks, out rightHandLandmarks, out poseWorldLandmarks, out leftHandWorldLandmarks, out rightHandWorldLandmarks, out segmentationMask, out poseRoi, false));
      }
      
      _rigAnimator.animate(poseWorldLandmarks, leftHandWorldLandmarks, rightHandWorldLandmarks);
    ...
    private void OnLeftHandWorldLandmarksOutput(object stream, OutputEventArgs<LandmarkList> eventArgs)
    {
      _poseWorldLandmarksAnnotationController.DrawLater(eventArgs.value);
    }
    
    private void OnRightHandWorldLandmarksOutput(object stream, OutputEventArgs<LandmarkList> eventArgs)
    {
      _poseWorldLandmarksAnnotationController.DrawLater(eventArgs.value);
    }
