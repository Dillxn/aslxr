...
namespace Mediapipe.Unity.Holistic
{
  public class HolisticTrackingGraph : GraphRunner
  {
    ...
    private RigAnimator _rigAnimator;
    ...
    public event EventHandler<OutputEventArgs<LandmarkList>> OnLeftHandWorldLandmarksOutput
    {
      add => _leftHandWorldLandmarksStream.AddListener(value);
      remove => _leftHandWorldLandmarksStream.RemoveListener(value);
    }
    
    public event EventHandler<OutputEventArgs<LandmarkList>> OnRightHandWorldLandmarksOutput
    {
      add => _rightHandWorldLandmarksStream.AddListener(value);
      remove => _rightHandWorldLandmarksStream.RemoveListener(value);
    }
    ...
    private const string _LeftHandWorldLandmarksStreamName = "left_hand_world_landmarks";
    private const string _RightHandWorldLandmarksStreamName = "right_hand_world_landmarks";
    ...
    private OutputStream<LandmarkListPacket, LandmarkList> _leftHandWorldLandmarksStream;
    private OutputStream<LandmarkListPacket, LandmarkList> _rightHandWorldLandmarksStream;
    ...
    public override void StartRun(ImageSource imageSource)
    {
      if (runningMode.IsSynchronous())
      {
        ...
        _leftHandWorldLandmarksStream.StartPolling().AssertOk();
        _rightHandWorldLandmarksStream.StartPolling().AssertOk();
    ...
    public override void Stop()
    {
      ...
      _leftHandWorldLandmarksStream?.Close();
      _leftHandWorldLandmarksStream = null;
      _rightHandWorldLandmarksStream?.Close();
      _rightHandWorldLandmarksStream = null;
    ...
    public bool TryGetNext(out Detection poseDetection, out NormalizedLandmarkList poseLandmarks, out NormalizedLandmarkList faceLandmarks, out NormalizedLandmarkList leftHandLandmarks,
                           out NormalizedLandmarkList rightHandLandmarks, out LandmarkList poseWorldLandmarks, out LandmarkList leftHandWorldLandmarks, out LandmarkList rightHandWorldLandmarks, out ImageFrame segmentationMask, out NormalizedRect poseRoi, bool allowBlock = true)
    {
      ...
      var r7 = TryGetNext(_leftHandWorldLandmarksStream, out leftHandWorldLandmarks, allowBlock, currentTimestampMicrosec);
      var r8 = TryGetNext(_rightHandWorldLandmarksStream, out rightHandWorldLandmarks, allowBlock, currentTimestampMicrosec);
      var r9 = TryGetNext(_segmentationMaskStream, out segmentationMask, allowBlock, currentTimestampMicrosec);
      var r10 = TryGetNext(_poseRoiStream, out poseRoi, allowBlock, currentTimestampMicrosec);
    ...
    protected override Status ConfigureCalculatorGraph(CalculatorGraphConfig config)
    {
      if (runningMode == RunningMode.NonBlockingSync)
      {
        ...
        _leftHandWorldLandmarksStream = new OutputStream<LandmarkListPacket, LandmarkList>(
            calculatorGraph, _LeftHandWorldLandmarksStreamName, config.AddPacketPresenceCalculator(_LeftHandWorldLandmarksStreamName), timeoutMicrosec);
        _rightHandWorldLandmarksStream = new OutputStream<LandmarkListPacket, LandmarkList>(
            calculatorGraph, _RightHandWorldLandmarksStreamName, config.AddPacketPresenceCalculator(_RightHandWorldLandmarksStreamName), timeoutMicrosec);
      ...
      else
      {
        ...
        _leftHandWorldLandmarksStream = new OutputStream<LandmarkListPacket, LandmarkList>(calculatorGraph, _LeftHandWorldLandmarksStreamName, true, timeoutMicrosec);
        _rightHandWorldLandmarksStream = new OutputStream<LandmarkListPacket, LandmarkList>(calculatorGraph, _RightHandWorldLandmarksStreamName, true, timeoutMicrosec);
